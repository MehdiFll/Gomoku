...
public abstract class GameSearch {
 ...
 /*
    ...
    public abstract Move createMove(Position startingPosition,int i);

    /*
     * Search utility methods:
     */
    protected Vector alphaBeta(int depth, Position p, boolean player, int niveau) {
        Vector v = alphaBetaHelper(depth, p, player, 1000000.0f, -1000000.0f, niveau);
        //System.out.println("^^ v(0): " + v.elementAt(0) + ", v(1): " + v.elementAt(1));
        return v;
    }

    protected Vector alphaBetaHelper(int depth, Position p,
            boolean player, float alpha, float beta, int niveau) {
        if (GameSearch.DEBUG) {
        ...
        for (int i = 0; i < moves.length; i++) {
            Vector v2 = alphaBetaHelper(depth + 1, moves[i], !player, -beta, -alpha, niveau);
...
/*
    public void playGameHumans(Position startingPosition, boolean humanPlayFirst) {
        while (true) {
            ...
            System.out.print("First human");
            Move move = createMove(startingPosition, i);
            startingPosition = makeMove(startingPosition, HUMAN, move);
            printPosition(startingPosition);
            System.out.print("Second humain");
            ...
            GomokuPosition old = (GomokuPosition) startingPosition;
            ...
            move = createMove(startingPosition);
    ...
    public void playGame(Position startingPosition, boolean humanPlayFirst) {
        ...
        while (true) {
            ...
            Move move = createMove(startingPosition);
